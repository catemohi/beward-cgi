...
from beward_cgi.date import DateModule
...
def get_date_dump(ip=None, username=None, password=None):
    ...
    """
    print("Get date dump  %s %s/%s" % (ip, username, password))
    if ip is None:
    ...
    client = DateModule(ip=ip, login=username, password=password)
    client.load_params()
...
if __name__ == "__main__":
    print(get_date_dump(ip="10.80.1.200"))
